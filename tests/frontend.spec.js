...
test.describe('Gyro Synth Frontend Tests', () => {
  ...
  test('should apply virtual orientation fallback on pointerdown and pointermove on desktop', async ({ page }) => {
    ...
    // Click on the SVG to trigger virtual fallback (while modal is closed)
    await page.mouse.move(box.x + box.width / 4, box.y + box.height / 4);
    await page.mouse.down();

    // Open settings modal to display/verify the values
    await page.keyboard.press('m');
    ...
    // Close settings modal so we can interact with visualizer again
    ...
    // Re-open settings modal to check values
    await page.keyboard.press('m');
    await expect(page.locator('#settingsModal')).toBeVisible();
