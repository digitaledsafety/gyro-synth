...
def test_js_logic_integrity():
    ...
    if 'await reverb.ready' not in content and 'await this.reverbNode.ready' not in content:
        print("Missing await reverb.ready / await this.reverbNode.ready")
        return False

    # Check for Filter
    if 'setFilterCutoff' not in content:
        print("Missing setFilterCutoff")
        return False
    if 'setFilterQ' not in content:
        print("Missing setFilterQ")
        return False
